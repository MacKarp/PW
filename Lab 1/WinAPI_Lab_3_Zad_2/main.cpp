...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdParam, int cmdShow)
{
	...
	hWnd = CreateWindowEx(0,                     /* extended window style */
						  ...
						  Window_Height,                   /* heigth of window */
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	PAINTSTRUCT ps;
	HDC hdc;
	static int cursor_x_position = 350, cursor_y_position = 250;
	static bool double_click_status = 0;

	switch (message)
	{
	case WM_PAINT:
		hdc = BeginPaint(hwnd, &ps);
		Draw_Window(hdc, double_click_status, cursor_x_position, cursor_y_position);
		EndPaint(hwnd, &ps);
		break;
	case WM_LBUTTONDOWN:
		cursor_x_position = LOWORD(lParam);
		cursor_y_position = HIWORD(lParam);
